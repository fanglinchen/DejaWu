...
function findLists(node) {
    ...
    if (children.length >= MIN_LIST_LENGTH) {
      ...
      children.forEach(child => {
        ...
        for (let i = 0; i < tagClusters.length; i++) {
          ...
          if (example.tagName != tag) continue;

          let exampleClasses = Array.from(example.classList);
          let childClasses = Array.from(child.classList);

          let intersection = exampleClasses.filter(
            className => childClasses.indexOf(className) > -1
          );
          ...
          if (
            (!exampleClasses.length && !childClasses.length) ||
  ...
  listItems = listItems.filter(
    group => ignoredLength(group) == parseInt(idealLength)
  );
  ...
  if (listItems.length) {
    ...
    if (area > 10000) {
      //If the area is bigger than the area of a 100 * 100px square, we include this list
...
export default function ListAutoSelector({ trigger, onComplete, onUpdate }) {
  ...
  return {
    ...
    onSelectionChange: function({ causingEvent, mouseDown, mousePosition }) {
      if (
        causingEvent == "mousedown" &&
        mouseDown.target &&
        mouseDown.target.className.includes("siphon-list-item")
      ) {
        mouseDown.preventDefault();
        mouseDown.stopPropagation();
        let listIdx = parseInt(
          mouseDown.target.className
            .split(" ")
            .find(name => name.includes("siphon-list_"))
            .split("_")[1]
        );
        let itemIdx = parseInt(
          mouseDown.target.className
            .split(" ")
            .find(name => name.includes("siphon-list-item_"))
            .split("_")[1]
        );

        let idx = selectedItems.findIndex(
          item => item[0] == listIdx && item[1] == itemIdx
        );
        if (idx > -1) {
        ...
        if (onUpdate && selectedItems.length) {
          onUpdate(
            selectedItems.map(item => lists[item[0]][item[1]]),
            remainingBoxes
          );
        }
      } else if (
        highlightedElement != mousePosition.target &&
        mousePosition.target.className.includes("siphon-list-item")
      ) {
        ...
        if (
          highlightedElement &&
          !highlightedElement.className.includes("siphon-included-item")
        ) {
    ...
    onSelectionStart: function(e) {
      ...
      if (!lists) lists = findLists(document.body);
      ...
      rects.forEach((list, listIdx) => {
        list.forEach((rect, itemIdx) => {
          let bounding = document.body.appendChild(
