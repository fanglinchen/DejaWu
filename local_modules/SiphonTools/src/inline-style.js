    ...
    for (let j = i + 1; j < cloneNodes.length; j++) {
      ...
      for (let v = 0; v < parents.elems.length; v++) {
        ...
        if (parent.contains(originalNodes[j])) {
          ...
          } else if (parent != originalNodes[j]) {
            //Otherwise we need to find the appropriate parent elements up to this one
            let addParents = getNodesToParents(
              originalNodes[j],
              cloneNodes[j],
              parent
            );
...
function getNodesToParents(elem, clone, parent) {
  ...
  while (elem.parentElement != parent) {
    ...
    let { element, styleInfo } = computedStyleToInlineStyle(parent, {
      clone: true,
      recursive: false
    });
  ...
  while (true) {
    ...
    if (
      [
        ...
        .includes(elem.tagName.toLowerCase())
    ) {
      //We have a list item, find the parent
...
function computedStyleToInlineStyle(element, options = {}) {
    ...
    Array.prototype.map.call(children, function(child, idx) {
      if (
        ...
        for (let index = child.attributes.length - 1; index >= 0; --index) {
          placeholder.attributes.setNamedItem(
            child.attributes[index].cloneNode()
          );
        }
        ...
        for (
          let index = clonedBody.attributes.length - 1;
          index >= 0;
          --index
        ) {
          placeholder.attributes.setNamedItem(
            clonedBody.attributes[index].cloneNode()
          );
        }
  ...
  for (var i = 0; i < computedStyle.length; i++) {
    ...
    if (!properties || properties.indexOf(property) >= 0) {
      ...
      if (
        initialValues[element.tagName.toLowerCase()] &&
        ...
        // }
        if (clone) {
          //Clear the styles first because sometimes they won't update with their resolved values
    ...
    for (var i = 0; i < computedStyle.length; i++) {
      ...
      if (!properties || properties.indexOf(property) >= 0) {
        ...
        if (
          ...
          initialValues["::before"][property] != value
        ) {
          if (property == "content") {
            //We need to escape content appropriately
            var ret = '"';
            for (let i = 1; i < value.length - 1; i++) {
              let charCode = value.charCodeAt(i);
              if (charCode < 128) ret += value.substr(i, i + 1);
              else ret += "\\" + charCode.toString(16);
            }
            value = ret + '"';
          }
  ...
  if (saveAfter) {
    ...
    for (var i = 0; i < computedStyle.length; i++) {
      ...
      if (!properties || properties.indexOf(property) >= 0) {
        ...
        if (
          initialValues["::after"] &&
          initialValues["::after"][property] != value
        ) {
          if (property == "content") {
            //We need to escape content appropriately
            var ret = '"';
  ...
  if (saveBefore || saveAfter) {
    if (clone) clone.classList.add(cssClass);
    else element.classList.add(cssClass);
  }
