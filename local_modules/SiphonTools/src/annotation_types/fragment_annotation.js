...
export default class FragmentAnnotation extends AnchoredAnnotation {
  constructor(nodes) {
    super(nodes[0]);
    this.nodes = nodes;
    this.paths = this.nodes.map(elem =>
      XPath.getUniqueXPath(elem, document.body)
    );
    ...
    let inlineNodes = this.nodes.map(node => {
      let { element, styleInfo } = computedStyleToInlineStyle(node, {
        recursive: true,
        clone: true
      });
    ...
    inlineNodes.forEach(node =>
      Array.from(node.querySelectorAll("style")).forEach(elem => elem.remove())
    );
    inlineNodes.forEach(node =>
      Array.from(node.querySelectorAll("script")).forEach(elem => elem.remove())
    );
    ...
    let { cloneNodes: resolved, style } = resolveHangingTags(
      this.nodes,
      inlineNodes.slice(0)
    );
    ...
    this.nodes.forEach(node => {
      if (node.tagName.toLowerCase().match(/^h[1-6]$/)) headerNodes.push(node);
  ...
  rehydrate() {
    this.nodes = compact(
      this.paths.map(path => XPath.find(path, document.body))
    );
